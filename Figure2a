...
psfile=Reconstructed-Kimberlites-for-the-last-210-Myr-M21-Anchor0-SMEAN2Background.ps
# psfile=Reconstructed-Kimberlites-for-the-last-210-Myr-M21-NNR-SMEAN2Background.ps
TomoGrid=SMEAN2-dvs.3.grd # 2775 km depth slice

gmt grdimage -R${region} -J${proj_map} ${TomoGrid} -CcolorsLLSVP.cpt -V -K > $psfile
gmt pscoast -R$region -J${proj_map} -Dl -W0.2p,black -O -V -K >> $psfile
...
gmt psxy $KimberlitesXY -R${region} -J${proj_map} -Sc0.2 -W0.0p,black -CcolorsKimb.cpt -O -V -K >> $psfile #1)
